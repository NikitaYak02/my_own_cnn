...
static BmmTileConfig g_current_tile_config{
    kBlockRows, kBlockCols, kBlockDepth, kThreadRows, kThreadCols
};

static bool is_valid_tile_config(BmmTileConfig cfg) {
    if (cfg.block_rows <= 0 || cfg.block_cols <= 0 || cfg.block_depth <= 0) {
        return false;
    }
    if (cfg.thread_rows == 0) {
        cfg.thread_rows = kThreadRows;
    }
    if (cfg.thread_cols == 0) {
        cfg.thread_cols = kThreadCols;
    }
    if (cfg.thread_rows <= 0 || cfg.thread_cols <= 0) {
        return false;
    }
    // Keep launcher dimensions realistic for a single CUDA block.
    if (cfg.thread_rows > 8 || cfg.thread_cols > 8) {
        return false;
    }
    if (cfg.block_rows % cfg.thread_rows != 0 || cfg.block_cols % cfg.thread_cols != 0) {
        return false;
    }
    const int threads_x = cfg.block_cols / cfg.thread_cols;
    const int threads_y = cfg.block_rows / cfg.thread_rows;
    if (threads_x <= 0 || threads_y <= 0 || threads_x * threads_y > 1024) {
        return false;
    }
    return true;
}

static BmmTileConfig normalized_tile_config(BmmTileConfig cfg) {
    if (cfg.thread_rows == 0) {
        cfg.thread_rows = kThreadRows;
    }
    if (cfg.thread_cols == 0) {
...
extern "C" int bmm_set_tile_config(const BmmTileConfig* config) {
    if (config == nullptr) {
        return 0;
    }
    BmmTileConfig cfg = normalized_tile_config(*config);
    if (!is_valid_tile_config(cfg)) {
        return 0;
    }
    g_current_tile_config = cfg;
    return 1;
}

extern "C" void bmm_get_tile_config(BmmTileConfig* config) {
    if (config == nullptr) {
        return;
    }
    *config = g_current_tile_config;
}

extern "C" void bmm_reset_tile_config(void) {
    g_current_tile_config =
        BmmTileConfig{kBlockRows, kBlockCols, kBlockDepth, kThreadRows, kThreadCols};
}

extern "C" void bmm_get_last_launch_info(BmmTileLaunchInfo* info) {
    if (info == nullptr) {
        return;
    }
    const BmmTileConfig cfg = g_current_tile_config;
    info->tile = cfg;
    info->threads_x = cfg.block_cols / cfg.thread_cols;
    info->threads_y = cfg.block_rows / cfg.thread_rows;
    info->shared_mem_bytes =
        static_cast<int>(((static_cast<size_t>(cfg.block_rows) * (cfg.block_depth + kSharedPad)) +
